...
export default function ImpactStatItem({ blok }: { blok: ImpactStatBlok }) {
  const ref = useRef<HTMLDivElement>(null);
  const [visible, setVisible] = useState(false);
  const target = Number(blok.value) || 0;
  const color = blok.color && VALUE_COLOR[blok.color] ? blok.color : "navy";
