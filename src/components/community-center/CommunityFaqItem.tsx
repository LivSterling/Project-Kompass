...
export type CommunityFaqItemBlok = {
  _uid: string;
  component?: string;
  question?: string;
  answer?: string;
...
export default function CommunityFaqItem({ blok }: { blok: CommunityFaqItemBlok }) {
  const [open, setOpen] = useState<boolean>(blok.default_open === true);
  const accent =
    blok.accent_color && ACCENT_BG[blok.accent_color] ? blok.accent_color : "orange";
    ...
    >
      ...
      {open && blok.answer ? (
        <div className="-mt-1 px-6 pb-5">
          <p className="max-w-[610px] text-[16px] font-medium leading-[26px] tracking-[0.01em] whitespace-pre-wrap text-[#151825]">
            {blok.answer}
          </p>
        </div>
      ) : null}
