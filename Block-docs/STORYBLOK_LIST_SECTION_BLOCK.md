...
| Field          | Type                     | Notes                                                                                        |
| -------------- | ------------------------ | -------------------------------------------------------------------------------------------- |
| `title`        | Text                     | Row/entry heading (also used as the bullet text in `bullets` style). **Preferred API name.** |
| `text`         | Text                     | Accepted alias for `title` (some spaces used this name when creating the blok).              |
...
> The component reads `title` first, then falls back to `text` / `headline` / `heading` /
> `name` / `label`. Prefer renaming the Storyblok field’s **API name** to `title` when you can.
